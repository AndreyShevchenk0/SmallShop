...
class NewCategoryView(View):
    """ вью для форми  создания новой категории  2-а метода get і post"""

    def get(self, request, *args, **kwargs):
        form = NewCategoryForm(request.POST or None)  # инстанцирование
        context = {'form': form}
...
class CreateNewFeature(View):
    """ вью для створення нових характеристик у формі """
...
class CreateFeatureView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not created:
            return JsonResponse({
                "error": f"Значення '{value}' вже ісуне."
            })
        messages.add_message(
            request, messages.SUCCESS,
            f'Значение "{value}" для характеристики '
            f'"{feature.feature_name}" у категоріі {category.name} успішно створено'
        )
    ...
    def get(self, request, *args, **kwargs):
        ...
        <hr>
            <div class="row">
                ...
                <div class="col-md-4">
                    <h4 class="text-center">Теперішне значення</h4>
                </div>
                <div class="col-md-4">
        ...
        <div class="col-md-4">
            <p class='text-center'><button class="btn btn-success" id="save-updated-features">Сохранити</button></p>
        </div>
        <div class="col-md-4">
        </div>
        </div>
...
class UpdateProductFeaturesAjaxView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        messages.add_message(
            request, messages.SUCCESS,
            f'Значення характеристик для товару {product.title} успішно оновлені'
        )
        return JsonResponse({"result": "ok"})
